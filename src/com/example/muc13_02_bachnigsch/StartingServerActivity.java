...
public class StartingServerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// TODO: register boradcast receiver for being notified when leaving discoverability mode
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == REQUEST_DISCOVERABLE_BT) {
			if(resultCode == RESULT_CANCELED) {
				Toast toast = Toast.makeText(getApplicationContext(), "Discoverability necessary in order to start a server", Toast.LENGTH_LONG);
				toast.show();
				finish();
			} else {
				Toast toast = Toast.makeText(getApplicationContext(), "Starting server...", Toast.LENGTH_LONG);
				toast.show();
	...
	private class ServerTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
	        ...
	        try {
	            ...
	            tmp = mBTAdapter.listenUsingRfcommWithServiceRecord("MUCubigame", UUID.fromString("4080ad8d-8ba2-4846-8803-a3206a8975be"));
	        } catch (IOException e) { }
	        mBTServerSocket = tmp;
		...
		protected void onPostExecute(Void result) {
			...
			Toast toast = Toast.makeText(getApplicationContext(), "Connection established!", Toast.LENGTH_LONG);
			toast.show();
		}
